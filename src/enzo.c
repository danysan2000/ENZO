...
static PopID parents    = 0, // NULL,
             offsprings = 0, // NULL,
             reference  = 0; // NULL;  /* contains the max. network 
...
static void broadcast_message( char *msg, int ignore_active, void *data )
{
    ...
    msgv[0] = msg;            msgc++;
    msgv[1] = (char *) data;  msgc++; /* one more supplementary data          */
      ...
      case MTYPE_PRE:
        {
            ...
            localTable = preTable; localCnt = preTable_cnt++;
            break;
        }
      case MTYPE_STOP:
        {
            ...
            localTable = stopTable; localCnt = stopTable_cnt++;
            break;
        }
      case MTYPE_SELECTION:
        {
            ...
            localTable = selectionTable; localCnt = selectionTable_cnt++;
            break;
        }
      case MTYPE_CROSSOVER:
        {
            ...
            localTable = crossoverTable; localCnt = crossoverTable_cnt++;
            break;
        }
      case MTYPE_MUTATION:
        {
            ...
            localTable = mutationTable; localCnt = mutationTable_cnt++;
            break;
        }
      case MTYPE_OPT:
        {
            ...
            localTable = optTable; localCnt = optTable_cnt++;
            break;
        }
      case MTYPE_EVAL:
        {
            ...
            localTable = evalTable; localCnt = evalTable_cnt++;
            break;
        }
      case MTYPE_HISTORY:
        {
            ...
            localTable = historyTable; localCnt = historyTable_cnt++;
            break;
        }
      case MTYPE_SURVIVAL:
        {
            ...
            localTable = survivalTable; localCnt = survivalTable_cnt++;
            break;
        }
      case MTYPE_POST:
        {
            ...
            localTable = postTable; localCnt = postTable_cnt++;
            break;
        }
      default: return( LT_UNKNOWN_TYPE );
    }
...
static void initialize_all( int argc, char *argv[] )
{
    ...
    if( argc < 2 ) { show_usage(  argv[0] ); exit( 0 ); }
    else           { cmd_file  =  argv[1];              }

    if( argc > 2 ) { log_file  =  argv[2];              }
    
    if( argc > 3 ) { rand_seed =  atoi( argv[3] );      }
    else           { rand_seed =  (int) time( NULL );   }
